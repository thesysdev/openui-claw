...
Agent ID (from agents.list)
  → session key: agent:<id>:main:openui-claw
    → one thread in the sidebar
      → chat.history fetches messages for that key
      → chat.send writes messages to that key
```

The `:openui-claw` suffix makes Claw's session isolated from other clients. Removing it would make it shared (same history as `openclaw chat` CLI).
