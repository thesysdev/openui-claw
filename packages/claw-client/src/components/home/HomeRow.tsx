...
export function HomeRow(props: HomeRowProps) {
  ...
  return isMobile ? <MobileRow {...props} /> : <DesktopRow {...props} />;
}

/** Original desktop layout — kept pixel-identical to pre-mobile state. */
function DesktopRow({
  icon: Icon,
  title,
  subtitle,
...
/** Mobile layout — flat row designed for the bordered list-card wrapper used
 *  by HomeView/AppsView/ArtifactsView/CronsView on mobile. */
function MobileRow({
  icon: Icon,
  title,
  subtitle,
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="group flex min-h-[64px] w-full items-center gap-m bg-transparent px-ml py-m text-left transition-colors duration-150 first:rounded-t-2xl last:rounded-b-2xl active:bg-sunk-light dark:active:bg-elevated-light"
    >
      <div
